...
We support two backends for transcription:

- Faster-whisper (local GPU inference): `pip install faster-whisper`
- OpenAI Whisper API (cloud-based): `pip install openai` and set the `OPENAI_API_KEY` environment variable.
...
Sentence buffer trimming requires a sentence segmenter. We support only English, Chinese, and Cantonese. Install with:
...
usage: whisper_online.py [-h] [--min-chunk-size MIN_CHUNK_SIZE] [--model {tiny.en,tiny,base.en,base,small.en,small,medium.en,medium,large-v1,large-v2,large-v3,large}] [--model_cache_dir MODEL_CACHE_DIR]
                         [--model_dir MODEL_DIR] [--lang LANG] [--task {transcribe,translate}] [--backend {faster-whisper,openai-api}] [--vac] [--vac-chunk-size VAC_CHUNK_SIZE] [--vad]
...
asr = FasterWhisperASR(lang, "large-v2")  # loads and wraps Whisper model
# set options:
# asr.set_translate_task()  # it will translate from lang into English
