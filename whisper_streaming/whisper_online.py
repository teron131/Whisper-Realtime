...
class OpenaiApiASR(ASRBase):
    ...
    def __init__(self, lang=None, temperature=0, logfile=sys.stderr):
        ...
        self.original_language = None if lang == "auto" else lang  # ISO-639-1 language code
    ...
    def transcribe(self, audio_data, prompt=None, *args, **kwargs):
        ...
        params = {"model": self.modelname, "file": buffer, "response_format": self.response_format, "temperature": self.temperature, "timestamp_granularities": ["word", "segment"]}
...
def create_tokenizer(lang):
    """Returns a sentence tokenizer for supported languages: English, Chinese, Cantonese."""
    supported = {"en", "zh", "yue"}
    if lang not in supported:
        raise ValueError(f"Sentence buffer trimming not supported for language {lang}")
...
def add_shared_args(parser):
    ...
    parser.add_argument("--lang", "--language", type=str, default="auto", help="Source language code, e.g. en,de,cs, or 'auto' for language detection.")
    parser.add_argument("--task", type=str, default="transcribe", choices=["transcribe", "translate"], help="Transcribe or translate.")
    parser.add_argument("--backend", type=str, default="faster-whisper", choices=["faster-whisper", "openai-api"], help="Load only this backend for Whisper processing.")
...
def asr_factory(args, logfile=sys.stderr):
    ...
    if backend == "openai-api":
        ...
        size = args.model
        t = time.time()
        logger.info(f"Loading Whisper {size} model for {args.lang}...")
        asr = FasterWhisperASR(modelsize=size, lang=args.lang, cache_dir=args.model_cache_dir, model_dir=args.model_dir)
        e = time.time()
